...
int main() {
  ...
  min_cost_flow_graph mcf(n+5);
  // source to rx
  mcf.add_edge(n, n+1, m, 0);
  mcf.add_edge(n, n+2, m, 0);
  mcf.add_edge(n, n+3, m, 0);
  // rx to pin
  for (int i = 0; i < n; i++) {
    mcf.add_edge(n+1, i, 1, 1000000000LL - ((a[i] * b[i]) % r[0]));
    mcf.add_edge(n+2, i, 1, 1000000000LL - ((a[i] * b[i] * b[i]) % r[1]));
    mcf.add_edge(n+3, i, 1, 1000000000LL - ((a[i] * b[i] * b[i] * b[i]) % r[2]));
  }
  // pin to sink (with penalties)
  for (int i = 0; i < n; i++) {
    mcf.add_edge(i, n+4, 1, (a[i] * b[i]));
    mcf.add_edge(i, n+4, 1, (a[i] * b[i] * b[i] - a[i] * b[i]));
    mcf.add_edge(i, n+4, 1, (a[i] * b[i] * b[i] * b[i] - a[i] * b[i] * b[i]));
  }

  auto res = mcf.flow(n, n+4);

  ll ans = 1000000000LL * (m * 3) - res.second;
