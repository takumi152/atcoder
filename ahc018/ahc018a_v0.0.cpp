...
double destroy_bedrock(position pos) {
  if (estimated_hardness[pos.x][pos.y] == -1.0) {
    ...
    const double hardness_increase_factor = 1.3;
    double current_hardness = base_hardness;
    int next_power = (int) current_hardness;
    while (true) {
      if (next_power <= 0) {
        double next_hardness = current_hardness * hardness_increase_factor;
        next_power = (int) next_hardness - (int) current_hardness;
    ...
    const double base_hardness_factor = 0.95;
    const double hardness_increase_factor = 1.05;
...
double calculate_estimated_cost(position pos) {
  const double hardness_factor = 0.0001;
  const double action_count_factor = 1.8;
