...
namespace HyperParameter {
  int leaf_length_limit = 3;
  double excess_leaf_length_factor = -0.00920551728970439;
  double base_temperature = 0.21613359963562462;
  double target_temperature = 0.017365019376191605;
  double base_score_factor = 1.0;
  double crop_lifetime_score_factor = 0.0005596887761322908;
  double plant_harvest_num_score_factor = -1.4458770843659197;
  double base_separation_penalty = 7.502356086447599;
  double separation_penalty_decay_rate = 0.3368303603138996;

  void load_hyper_parameter(int argc, char *argv[]) {
    if (argc > 1) leaf_length_limit = atoi(argv[1]);
    if (argc > 2) excess_leaf_length_factor = strtod(argv[2], nullptr);
    if (argc > 3) base_temperature = strtod(argv[3], nullptr);
    if (argc > 4) target_temperature = strtod(argv[4], nullptr);
    if (argc > 5) base_score_factor = strtod(argv[5], nullptr);
    if (argc > 6) crop_lifetime_score_factor = strtod(argv[6], nullptr);
    if (argc > 7) plant_harvest_num_score_factor = strtod(argv[7], nullptr);
    if (argc > 8) base_separation_penalty = strtod(argv[8], nullptr);
    if (argc > 9) separation_penalty_decay_rate = strtod(argv[9], nullptr);
  }
}
...
Output solve(const Input& input) {
  struct SAState {
    ...
    vector<vector<bitset<4>>> dir_is_child;
    vector<vector<int>> leaf_length;
    // vector<vector<int>> tree_depth;

    const int leaf_length_limit = HyperParameter::leaf_length_limit;

    int total_leaf_score = 0;
    int excess_leaf_length_score = 0;

    const double total_leaf_factor = 1e0;
    const double excess_leaf_length_factor = HyperParameter::excess_leaf_length_factor;

    SAState(const Input& input): input(input) {
      parent_dir = vector<vector<int>>(input.board_size, vector<int>(input.board_size, -1));
      dir_is_child = vector<vector<bitset<4>>>(input.board_size, vector<bitset<4>>(input.board_size));
      {
        vector<vector<bool>> visited(input.board_size, vector<bool>(input.board_size));
        queue<pair<Position, int>> que;
        que.emplace(input.entrance_pos, -1);
        while (!que.empty()) {
          auto [pos, dir] = que.front();
          que.pop();
          if (visited[pos.x][pos.y]) continue;
          visited[pos.x][pos.y] = true;
          parent_dir[pos.x][pos.y] = dir;
          if (dir != -1) dir_is_child[pos.x + Adj4::dx[dir]][pos.y + Adj4::dy[dir]][Adj4::reverse(dir)] = true;
          for (int d = 0; d < 4; d++) {
            if (is_adjacent(input, pos, d) && !visited[pos.x + Adj4::dx[d]][pos.y + Adj4::dy[d]]) {
              que.emplace(Position(pos.x + Adj4::dx[d], pos.y + Adj4::dy[d]), Adj4::reverse(d));
            }
          }
        }
      }

      leaf_length = vector<vector<int>>(input.board_size, vector<int>(input.board_size));
      for (int i = 0; i < input.board_size; i++) {
        for (int j = 0; j < input.board_size; j++) {
          if (dir_is_child[i][j].none()) {
            Position now(i, j);
            int len = 0;
            while (dir_is_child[now.x][now.y].count() <= 1) {
              len++;
              int d = parent_dir[now.x][now.y];
              if (d == -1) break;
              now.x += Adj4::dx[d];
              now.y += Adj4::dy[d];
            }
      ...
      // tree_depth = vector<vector<int>>(input.board_size, vector<int>(input.board_size));
      // {
      //   queue<pair<Position, int>> que;
      //   que.emplace(input.entrance_pos, 0);
      //   while (!que.empty()) {
      //     auto [now_pos, now_depth] = que.front();
      //     que.pop();
      //     tree_depth[now_pos.x][now_pos.y] = now_depth;
      //     for (int d = 0; d < 4; d++) {
      //       if (dir_is_child[now_pos.x][now_pos.y][d]) {
      //         que.emplace(Position(now_pos.x + Adj4::dx[d], now_pos.y + Adj4::dy[d]), now_depth + 1);
      //       }
      //     }
      //   }
      // }
    ...
    SAState(const Input& input, vector<string> initial_tree): input(input) {

    }

    bool change_parent_dir(const Position pos, const int new_dir) {
      assert(0 <= new_dir && new_dir < 4);
      if (parent_dir[pos.x][pos.y] == -1) return false; // rootなので親が無い
      if (parent_dir[pos.x][pos.y] == new_dir) return false; // 元の向きと同じなので無意味
      if (!is_adjacent(input, pos, new_dir)) return false; // 壁がある方向なので親に指定できない
      if (dir_is_child[pos.x][pos.y][new_dir]) return false; // 子がある方向なので親に指定できない

      if (is_descendant(pos, Position(pos.x + Adj4::dx[new_dir], pos.y + Adj4::dy[new_dir]))) {
        // 新しい親が、操作前の時点で自身の子孫であった場合、根と連結ではなくなってしまうので操作できない
        return false;
      }
      else {
        // 逆に、そうでない場合は連結性が必ず保たれるので操作できる
        int orig_parent_dir = parent_dir[pos.x][pos.y];
        parent_dir[pos.x][pos.y] = new_dir;
        dir_is_child[pos.x + Adj4::dx[orig_parent_dir]][pos.y + Adj4::dy[orig_parent_dir]][Adj4::reverse(orig_parent_dir)] = false;
        dir_is_child[pos.x + Adj4::dx[new_dir]][pos.y + Adj4::dy[new_dir]][Adj4::reverse(new_dir)] = true;

        update_new_parent(Position(pos.x + Adj4::dx[new_dir], pos.y + Adj4::dy[new_dir]));
        update_leaf_length_and_score(Position(pos.x + Adj4::dx[orig_parent_dir], pos.y + Adj4::dy[orig_parent_dir]));
        for (int d = 0; d < 4; d++) {
          if (dir_is_child[pos.x + Adj4::dx[new_dir]][pos.y + Adj4::dy[new_dir]][d]) {
            update_leaf_length_and_score(Position(pos.x + Adj4::dx[new_dir] + Adj4::dx[d], pos.y + Adj4::dy[new_dir] + Adj4::dy[d]));
          }
        }
        return true;
      }
    ...
    // targetがbaseの子孫かどうか判定する
    // baseがtargetの先祖かどうかを判定するのと同値(のはず)なので、そちらで判定をしている
    bool is_descendant(const Position& pos_base, const Position& pos_target) {
      Position pos_now = pos_target;
      while (parent_dir[pos_now.x][pos_now.y] != -1) {
        int next_dir = parent_dir[pos_now.x][pos_now.y];
        pos_now.x += Adj4::dx[next_dir];
        pos_now.y += Adj4::dy[next_dir];
        if (pos_now == pos_base) return true;
      }
      return false;
    }

    void update_new_parent(const Position& parent_pos) {
      total_leaf_score -= leaf_length[parent_pos.x][parent_pos.y];
      excess_leaf_length_score -= max(0, abs(leaf_length[parent_pos.x][parent_pos.y] - leaf_length_limit) * (leaf_length[parent_pos.x][parent_pos.y] - leaf_length_limit));
      leaf_length[parent_pos.x][parent_pos.y] = 0;
      excess_leaf_length_score += max(0, abs(leaf_length[parent_pos.x][parent_pos.y] - leaf_length_limit) * (leaf_length[parent_pos.x][parent_pos.y] - leaf_length_limit));
    }

    void update_leaf_length_and_score(const Position& base_pos) {
      auto now_pos = base_pos;
      int len = 0;
      while (dir_is_child[now_pos.x][now_pos.y].count() == 1) {
        int next_child_dir = __builtin_ctz(dir_is_child[now_pos.x][now_pos.y].to_ulong());
        now_pos.x += Adj4::dx[next_child_dir];
        now_pos.y += Adj4::dy[next_child_dir];
        len++;
      }
      if (dir_is_child[now_pos.x][now_pos.y].none()) {
        auto leaf_pos = now_pos;

        total_leaf_score -= leaf_length[leaf_pos.x][leaf_pos.y];
        excess_leaf_length_score -= max(0, abs(leaf_length[leaf_pos.x][leaf_pos.y] - leaf_length_limit) * (leaf_length[leaf_pos.x][leaf_pos.y] - leaf_length_limit));

        now_pos = base_pos;
        while (dir_is_child[now_pos.x][now_pos.y].count() <= 1) {
          len++;
          int next_parent_dir = parent_dir[now_pos.x][now_pos.y];
          if (next_parent_dir == -1) break;
          now_pos.x += Adj4::dx[next_parent_dir];
          now_pos.y += Adj4::dy[next_parent_dir];
        }
        leaf_length[leaf_pos.x][leaf_pos.y] = len;
    ...
    void update_score_full() {
      total_leaf_score = 0;
      excess_leaf_length_score = 0;
      for (int i = 0; i < input.board_size; i++) {
        for (int j = 0; j < input.board_size; j++) {
          if (dir_is_child[i][j].none()) {
            total_leaf_score += leaf_length[i][j];
            excess_leaf_length_score += max(0, abs(leaf_length[i][j] - leaf_length_limit) * (leaf_length[i][j] - leaf_length_limit + 1) / 2);
          }
        }
      }
    }
    ...
    double get_score() {
      return (double) total_leaf_score * total_leaf_factor +
             (double) excess_leaf_length_score * excess_leaf_length_factor;
    }

    void print_parent_dir() {
      for (int i = 0; i < input.board_size; i++) {
        for (int j = 0; j < input.board_size; j++) {
               if (parent_dir[i][j] == 0) cerr << "^";
          else if (parent_dir[i][j] == 1) cerr << "<";
          else if (parent_dir[i][j] == 2) cerr << "v";
          else if (parent_dir[i][j] == 3) cerr << ">";
          else                            cerr << "*";
        }
        cerr << endl;
      }
