...
vector<vector<bool> > board_base(n, vector<bool>(n));
...
int calc_score2() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      if (board_base[i][j] && !board_placement[i][j]) score += poly_cost[0];
    }
  }
...
void solve() {
    ...
    for (int g = 0; g < group_count-1; g++) {
      vector<Pii> in_group;
      {
        ...
        while (!que.empty()) {
          ...
          for (int d = 0; d < 4; d++) {
            if (nx + dx[d] < 0 || nx + dx[d] >= n || ny + dy[d] < 0 || ny + dy[d] >= n) continue;
            if ((board_placement[nx + dx[d]][ny + dy[d]] || board_base[nx + dx[d]][ny + dy[d]]) && !visited[nx + dx[d]][ny + dy[d]]) {
  ...
  {
    int base_placement_count = board_place_info.size();
    ...
    while (theTimer.time() < time_limit) {
      ...
      if (roll < 0.50) {
        ...
        if (board_place_info.size() <= base_placement_count) continue;
        int idx = theRandom.nextUIntMod(board_place_info.size() - base_placement_count) + base_placement_count;
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      if (board_base[i][j] && !board_placement[i][j]) place_poly(0, i, j);
    }
  }
